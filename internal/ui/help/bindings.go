...
var sections = []section{
	{
		...
		Bindings: []binding{
			...
			{"Ctrl+f", "Find references via the language server (pre-filled with the word under the cursor); falls back to a content search with no server or no results"},
			...
			{"Ctrl+j", "Open the action popup: search and run any command"},
		},
	},
	{
		Title: "Splits",
	...
	},
	{
		...
		Bindings: []binding{
			...
			{"Tab", "Insert a tab, or spaces per this file's indent style (Insert mode)"},
			{"Ctrl+Alt+t", "Toggle this file between spaces and tabs"},
			...
			{"Ctrl+b", "Jump back to before the last jump (go-to-definition/-parent, a search match, :<line>, or a finder result)"},
	...
	},
	{
		...
		Bindings: []binding{
			...
			{"Left Right Home End", "Move the caret in the query (e.g. to add a prefix to a pre-filled word)"},
			{"Ctrl+k", "Edit the folder scope (opening while a folder is selected in the sidebar pre-fills it; Enter returns to the query)"},
			{"Esc", "Close"},
		},
	},
	{
		...
		Bindings: []binding{
			...
			{"Ctrl+k", "Edit the folder scope (Enter returns to Find)"},
			{"Esc", "Close"},
		},
	},
	{
