...
REPO="bricejulia/nib"
INSTALL_DIR="${NIB_INSTALL_DIR:-/usr/local/bin}"
...
ARCHIVE="nib_${VERSION}_${OS}_${ARCH}.tar.gz"
...
if [ ! -f "${TMPDIR}/nib" ]; then
    fail "binary not found in archive"
fi

printf "Installing to %s...\n" "$INSTALL_DIR"
if [ -w "$INSTALL_DIR" ]; then
    mv "${TMPDIR}/nib" "${INSTALL_DIR}/nib"
else
    sudo mv "${TMPDIR}/nib" "${INSTALL_DIR}/nib"
fi
chmod +x "${INSTALL_DIR}/nib"

printf "nib %s installed successfully!\n" "$VERSION"